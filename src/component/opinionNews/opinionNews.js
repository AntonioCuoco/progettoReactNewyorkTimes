...
export default function OpinionNews() {
    ...
    React.useEffect(() => {
        fetch("https://api.nytimes.com/svc/news/v3/content/nyt/opinion.json?api-key=ckWbHJh5FHfknu1LNNiwRv03F8tdT1og")
    ...
    const mapItem = article.map(item => {
        ...
        <div className="article-container5">
            <Main4
                ...
                titolo2={item.results[8].title}
                abstract2={item.results[8].abstract}
                url2={item.results[8].url}
                multimedia2={item.results[8].multimedia[0].url}
            ...
            <Sidebar2
                ...
                url4={item.results[9].url}
                multimedia4={item.results[9].multimedia[0].url}
